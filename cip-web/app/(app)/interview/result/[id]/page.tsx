    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p style={{ color: '#A1A1AA' }}>Loading interview results...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-8 max-w-5xl">
      ...
      <div className="rounded-3xl p-6 border text-center relative overflow-hidden"
        ...
        <p className="text-sm mb-2" style={{ color: '#A1A1AA' }}>Overall Score</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="font-semibold" style={{ fontFamily:'Syne,sans-serif', color:'#FFFFFF' }}>Score Breakdown</h3>
          {[
        ...
        <div className="rounded-2xl p-5 border" style={{ background: '#0A0A0A', borderColor: 'rgba(255,255,255,0.08)' }}>
          <h3 className="font-semibold mb-1" style={{ fontFamily:'Syne,sans-serif', color:'#FFFFFF' }}>Performance Radar</h3>
          <p className="text-xs mb-4" style={{ color: '#71717A' }}>Visual breakdown of your interview dimensions</p>
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
              <PolarGrid stroke="rgba(255,255,255,0.06)" />
              <PolarAngleAxis dataKey="subject" tick={{ fill: '#A1A1AA', fontSize: 12 }} />
              <Tooltip
                contentStyle={{ background:'#0A0A0A', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', color:'#FFFFFF' }}
              />
      ...
      {r.mistakes.length > 0 && (
        <div className="rounded-2xl p-5 border" style={{ background: '#0A0A0A', borderColor: 'rgba(255,255,255,0.08)' }}>
          <h3 className="font-semibold mb-4 flex items-center gap-2" style={{ fontFamily:'Syne,sans-serif', color:'#FFFFFF' }}>
          ...
          <div className="space-y-3">
            {r.mistakes.map((m: { question: string; feedback: string }, i: number) => (
              <div key={i} className="rounded-xl p-4 border"
                ...
                <p className="text-xs" style={{ color: '#A1A1AA' }}>{m.feedback}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          {r.suggestions.map((s: string, i: number) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl border"
              ...
              <p className="text-sm" style={{ color: '#FFFFFF' }}>{s}</p>
            </div>
          ))}
        </div>
      </div>
